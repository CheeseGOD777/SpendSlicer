"""Locate the prebuilt React bundle across the three ways CostSight runs.

The dashboard is a static Vite build that the FastAPI app serves at /app.
Where those files live depends on how CostSight was started:
...
1. **Frozen desktop build** (PyInstaller). Everything is unpacked into a
   ...
   bundle is collected there under ``costsight/web/static``.
2. **Installed wheel** (``pip install costsight``). The bundle ships inside
   the package as declared by ``[tool.setuptools.package-data]``, so it sits
   next to this module at ``costsight/web/static``.
3. **Source checkout** (``./run.sh``). Same location — Vite is configured to
   build straight into ``costsight/web/static`` rather than ``frontend/dist``,
...
def _frozen_static() -> Path | None:
    ...
    if not base:
        return None
    return Path(base) / "costsight" / "web" / "static"
