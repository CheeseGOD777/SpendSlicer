...
"""PyInstaller spec for the CostSight desktop builds.
...
Produces dist/CostSight.app on macOS and dist/CostSight/CostSight.exe on
...
static_dir = REPO / "costsight" / "web" / "static"
if not (static_dir / "index.html").is_file():
    raise SystemExit(
        "costsight/web/static/index.html is missing — run `cd frontend && npm run build` first."
    )
datas.append((str(static_dir), "costsight/web/static"))
...
hiddenimports += collect_submodules("costsight")
...
if sys.platform == "darwin":
    app = BUNDLE(
        ...
        bundle_identifier="io.costsight.desktop",
        info_plist={
            "CFBundleName": "CostSight",
            "CFBundleDisplayName": "CostSight",
            "CFBundleShortVersionString": os.environ.get("COSTSIGHT_VERSION", "0.3.0"),
            "CFBundleVersion": os.environ.get("COSTSIGHT_VERSION", "0.3.0"),
            "NSHighResolutionCapable": True,
            # No server, no inbound listener beyond loopback, no camera/mic.
            "LSMinimumSystemVersion": "11.0",
            "NSHumanReadableCopyright": "MIT licensed. © CostSight contributors.",
        },
    )
