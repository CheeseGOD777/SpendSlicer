...
def test_unmatched_ec2_bucket_becomes_aggregate_row_not_drift():
    from spendslicer.resources.ec2 import _attribute_from_usage_type
...
def test_eip_not_rescaled_and_vpc_aggregate_reduced():
    from spendslicer.resources import runner as R
...
def test_auto_assigned_public_ips_get_rows_at_exact_rate():
    from spendslicer.resources.eip import auto_assigned_ip_rows
...
def test_report_dict_has_real_counts_not_caps():
    from spendslicer.web.routes.export_api import _assemble_report
...
def test_stopped_rds_still_bills_storage():
    from spendslicer.resources.rds import _row_for_db
...
def test_region_to_location_covers_newer_regions():
    from spendslicer.core.pricing import _region_to_location
...
def test_planned_budget_limit_picks_current_period():
    from spendslicer.audit.budgets import _budget_limit
...
def test_ebs_monthly_estimate_is_type_aware():
    from spendslicer.audit.idle import _ebs_monthly_estimate
...
def test_stopped_rds_estimate_bills_storage():
    from spendslicer.audit.idle import _rds_stopped_estimate
...
def test_idle_eip_estimate_uses_730_hours():
    from spendslicer.audit.idle import _EIP_MONTHLY_USD
