"""AWS Pricing API wrapper with in-memory TTL cache and ap-south-1 fallback table.

The Pricing API is only available in us-east-1 / ap-south-1. We use us-east-1
for stability. Rates cached 24h. If the API is unreachable, fall back to a
hard-coded ap-south-1 on-demand rate table (as of 2024-2026).
"""
...
def ebs_rate(session: boto3.Session, volume_type: str, region: str = "ap-south-1") -> float:
    ...
    location = _region_to_location(region)
    api_type = {"gp3": "General Purpose", "gp2": "General Purpose",
                "io1": "Provisioned IOPS", "io2": "Provisioned IOPS",
                "st1": "Throughput Optimized HDD",
                "sc1": "Cold HDD",
                "standard": "Magnetic"}.get(volume_type, "General Purpose")
    price = _fetch_from_api(session, "AmazonEC2", [
