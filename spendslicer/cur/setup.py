...
from spendslicer.cur.schema import connect, get_parquet_glob

log = logging.getLogger("spendslicer.cur.setup")


def _db_path() -> Path:
    return Path(os.environ.get(
        "SPENDSLICER_CUR_DB",
        str(Path.home() / ".cache" / "spendslicer" / "cur.duckdb"),
    ))


def _local_dir() -> Path:
    return Path(os.environ.get(
        "SPENDSLICER_CUR_PARQUET_DIR",
        str(Path.home() / ".cache" / "spendslicer" / "parquet"),
    ))
...
def cmd_status(_args) -> int:
    ...
    if not glob:
        print("CUR not configured. Run `spendslicer cur ingest --bucket … --prefix …` first.")
        return 1
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="spendslicer cur")
