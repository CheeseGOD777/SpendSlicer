...
from spendslicer.aws.session import list_profiles, make_session
...
_TTL = float(os.environ.get("SPENDSLICER_CACHE_TTL_SECONDS", "1800"))          # 30 min fresh
_SWR_WINDOW = float(os.environ.get("SPENDSLICER_CACHE_SWR_SECONDS", "21600"))  # +6h stale
_CACHE_DIR = Path(os.environ.get("SPENDSLICER_CACHE_DIR", Path.home() / ".cache" / "spendslicer"))
...
# ---------------------------------------------------------------------------
_refresh_pool = ThreadPoolExecutor(max_workers=4, thread_name_prefix="spendslicer-refresh")
...
_heavy_pool = ThreadPoolExecutor(max_workers=4, thread_name_prefix="spendslicer-refresh-heavy")
...
_MAX_INFLIGHT_REFRESHES = int(os.environ.get("SPENDSLICER_MAX_INFLIGHT_REFRESHES", "32"))
...
def schedule_refresh(key: str, producer, heavy: bool = False) -> None:
    ...
    def _run() -> None:
        ...
        try:
            from spendslicer.web.middleware import set_new_counter
            set_new_counter()
        except Exception:
            pass
        try:
...
def available_periods() -> list[dict]:
    ...
    """
    from spendslicer.core.time_windows import recent_months
...
_CUR_DB_PATH = Path(os.environ.get("SPENDSLICER_CUR_DB", str(_CACHE_DIR / "cur.duckdb")))
...
def get_cur_store():
    ...
    if not _CUR_DB_PATH.exists():
        return None
    try:
        from spendslicer.cur.schema import (
...
def get_cost_source(session: boto3.Session):
    ...
    from spendslicer.aws.cost_store import CostStore
